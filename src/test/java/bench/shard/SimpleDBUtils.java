...
public class SimpleDBUtils {
	...
	// Name of attribute used to store item name hash, for slicing a shard into
	// chunks.
	public static final String ITEM_HASH_ATTR_NAME = SimpleDBUtils.class
			.getSimpleName() + "-itemHash";
	...
	private static final String NEGATIVE_HASH_FORMAT = "0%0" + HASH_DIGITS
			+ "d";
	private static final String POSITIVE_HASH_FORMAT = "1%0" + HASH_DIGITS
			+ "d";

	// These two must be kept in sync.
	private static final String DOMAIN_NAME_FORMAT = "%s-%d-of-%d";
	private static final Pattern DOMAIN_NAME_PATTERN = Pattern
			.compile("(.+)-(\\d+)-of-(\\d+)");

	public static List<String> getShardNames(String baseDomainName,
			int numShards) {
		List<String> result = new ArrayList<String>(numShards);
		for (int i = 1; i <= numShards; i++) {
			result.add(String.format(DOMAIN_NAME_FORMAT, baseDomainName, i,
					numShards));
		}
	...
	public static int getNumShardsForTable(SimpleDB sdb, String table)
			throws IOException, AWSException, InterruptedException {
		int numShards = 0;
				...
				if ((totalShards != -1) && (shardNumber != -1)) {
					if (numShards == 0) {
						...
						throw new IllegalStateException(
								String.format(
										"Table %s has shard %s with a different total count than a previous shard",
										table, domain));
					}
		...
		for (int i = 0; i < numShards; i++) {
			if (!shardFound[i]) {
				throw new IllegalStateException(String.format(
						"Table %s is missing shard #%d", table, i + 1));
			}
		}
	...
	public static int getShardIndex(String itemName, int numShards) {
		if (numShards == 1) {
			return 0;
		}

		long shardRange = HASH_RANGE / numShards;
		long hash = joaat_hash(itemName);
	...
	/**
	 * Return the total count for items in <domainName>, optionally selected
	 * with <expression> and limited to <limit> items.
	 * 
	 * @param sdb
	 * @param domainName
	 * @param expression
	 *            expression to use with selection, or ""/null.
	 * @param limit
	 *            limit to result, or NO_SELECT_LIMIT
	 * @return count of total items matching selection criteria
	 * @throws IOException
	 * @throws AWSException
	 * @throws InterruptedException
	 */
	public static int getItemCount(SimpleDB sdb, String domainName,
			String expression, int limit) throws IOException, AWSException,
			InterruptedException {

		int result = 0;
		String nextToken = null;

		// FUTURE KKr - if there's no query, then just make a metadata call as
		// that's
		// going to be faster than the select.

		String selectStr = String.format("select count(*) from `%s`",
				domainName);
		...
		// TODO KKr - seems like the select call will keep returning results
		// past what
		// we specified with the limit parameter...verify that this is the
		// expected behavior.
		// I think the fix is to reduce the limit by the returned count, each
		// time through
		// the loop.
		do {
			List<Map<String, String[]>> selectResult = sdb.select(selectStr,
					nextToken);
			...
			if (numItemsStr == null) {
				throw new RuntimeException(
						"SimpleDB select didn't return count");
			}
			...
			try {
				...
				throw new RuntimeException(
						"SimpleDB select returned invalid count: "
								+ numItemsStr);
			}
	...
	public static void deleteTable(SimpleDB sdb, String table)
			throws IOException, AWSException, InterruptedException {
		List<String> domains = sdb.listDomains();
		for (String domain : domains) {
			if (domain.startsWith(table)) {
